...
class UserServiceImplTest {
    ...
    void testGetUserIdFromAuthentication_Success() {
        ...
        when(jwtService.extractUserId(extractedToken)).thenReturn(expectedUserId);

        UUID result = userService.getUserIdFromAuthentication(authentication);

        assertEquals(expectedUserId, result);
        verify(authentication).getCredentials();
        verify(jwtService).extractUserId(extractedToken);
    }

    @Test
    void testGetUserIdFromAuthentication_InvalidToken() {
        when(authentication.getCredentials()).thenReturn("InvalidToken");
        assertThrows(RuntimeException.class, () -> userService.getUserIdFromAuthentication(authentication));
    ...
    @Test
    void testGetUserIdFromAuthentication_NullToken() {
